...
<template>
  <section
    ...
    id="voice-to-text"
    role="tabpanel"
    aria-labelledby="voice-to-text-link"
    tabindex="0"
  >
      ...
      >
        ...
        >
          <font-awesome-icon
            v-if="!isRecording"
            :icon="faMicrophone"
            class="start-icon text-center fs-3 px-1"
          />
          <font-awesome-icon
            v-else
            :icon="faStop"
            class="stop-icon fs-3 text-center px-1"
          />
        </button>
        <span v-if="!isRecording" class="fs-7 m-0">Start Recording</span>
        <span v-else class="fs-7 m-0">Recording...</span>
        <p class="fs-8 mb-4">
          <span class="fw-medium">Note:</span> You can only record in English
          Language
        </p>
        <div
          class="text-output position-relative card h-50 text-start w-75 shadow-sm p-4"
          rows="10"
          readonly
        >
          <span class="recorded-text">{{ transcript }}</span>
          <div
            class="command-button gap-2 d-flex align-items-center position-absolute"
          >
            <button
            ...
            >
              ...
              <font-awesome-icon :icon="faArrowRotateLeft" class="fs-6" />
            </button>
            <button

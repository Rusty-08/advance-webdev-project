...
let rgbaValue = computed({
  get: () =>
    `rgba(${R.value}, ${G.value}, ${B.value}, ${
      A.value <= 1 && A.value >= 0 ? A.value : 1
    })`,
  set: (newValue) => {
    const rgbaParts = newValue
      .match(/\d+(\.\d+)?/g)
...
const rgbaToHex = () => {
  ...
  const hexA = Math.round(A.value * 255)
    .toString(16)
    .padStart(2, "0");
...
<template>
  <section
    ...
    id="rgba-to-hex"
    role="tabpanel"
    aria-labelledby="rgba-to-hex-link"
    tabindex="0"
  >
    <div
      class="container-fluid p-3 px-4 vh-100 d-flex justify-content-center align-items-center"
    >
      <div
      ...
      >
        ...
        >
          <div class="rgb-result w-100 d-flex align-items-center flex-column">
            ...
            <div
              class="hex-result gap-5 p-3 mb-4 rounded-3 d-flex align-items-center"
            >
              <p class="hex mb-0 px-2 w-100 text-start fs-5 fw-medium">

...
const sidebar = [
  {
    category: "CONVERSION",
    links: ["Hex to RGBA", "RGBA to Hex", "Voice to Text"],
    icon: [faFillDrip, faBrush, faMicrophoneLines],
  },
  {
    category: "FLEXBOX",
    links: ["Flexbox Visualizer"],
    icon: [faCubes],
  },
];
...
<template>
  ...
  >
    ...
    <div class="logo-header d-flex align-items-center px-4">
      ...
      <h5 class="fs-6 m-0" v-show="!isSidebarShrinked">MICRO TOOLS</h5>
    </div>

    <!-- SIDEBAR LINKS -->
    <div
      class="nav flex-column nav-pills px-1"
      id="pills-tab"
      role="tablist"
    ...
    >
      <ul class="navbar-nav w-100 px-2">
        <template v-for="section in sidebar">
          <span class="nav-link-header" v-if="!isSidebarShrinked">
            {{ section.category }}
          </span>
          <font-awesome-icon
            v-else
            :icon="faEllipsis"
            class="nav-link-header ellipisis"
          />
    ...
    <button class="minimizerBtn btn" @click="toggleSidebarWidth()">
      <font-awesome-icon
        :icon="faChevronLeft"

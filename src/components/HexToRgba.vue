...
const updateColorValues = () => {
  let removedHash =
    hex.value.charAt(0) != "#" ? hex.value.trim() : hex.value.trim().slice(1);

  if (removedHash.length == 3 || removedHash.length == 4)
    removedHash = transformHex(removedHash);
...
<template>
  <section
    ...
    id="hex-to-rgba"
    role="tabpanel"
    aria-labelledby="hex-to-rgba-link"
    tabindex="0"
  >
    <div class="container-fluid vh-100 p-4 py-3">
      <div
        class="page-content shadow-sm gap-3 card p-4 w-100 h-100 d-flex justify-content-center align-items-center"
      >
        ...
        />
        <p class="fs-8 m-0">
          <span class="fw-medium">Tip:</span> You can enter 8 character Hex code
          — #11223344 for transparency.
        </p>
        <div
          class="result card w-100 p-4 d-flex justify-content-center align-items-center"
        >
          <div class="rgb-result w-50 d-flex flex-column">
            <h3 class="fs-6 text-center mb-3">CONVERTED</h3>
            <div
              class="rgb-wrapper d-flex w-100 px-3 rounded-3 justify-content-between align-items-center"
            >
              <p class="rgb mb-0 w-100 px-2 text-start fs-5 fw-medium">
            ...
            <div
              class="rgb-wrapper mt-3 d-flex w-100 px-3 mb-3 rounded-3 justify-content-between align-items-center"
            >
              <p class="rgba mb-0 w-100 px-2 text-start fs-5 fw-medium">
                <font-awesome-icon :icon="faFill" class="fs-5 px-1" />
                {{ rgba }}
              </p>
              <button
          ...
          <div
            :style="{
              backgroundColor: rgba != 'Invalid Input' ? rgba : transparent,
            }"
